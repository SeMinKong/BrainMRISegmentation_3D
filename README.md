...
웹은 세 질문 순서로 짜여 있습니다. **모델이 전체적으로 얼마나 맞는가 → 어느 검사에서 틀리는가 → 그 검사에서 어디가 틀렸는가.** 의료 지식이 없어도 읽을 수 있도록 모든 전문 용어 옆에 `?` 설명이 있고, 하단에 용어 설명 모음이 있습니다. UI는 밝은 단색 표면에 그림자 쌍으로 깊이를 주는 소프트 UI(뉴모피즘) 스타일입니다.

| 화면 | 기능 |
| --- | --- |
| **모델 성능(첫 화면)** | 시작 체크리스트, **남은 n개 검사 전체 예측** 버튼(순차 큐, 진행률), 검증 검사 평균 일치도(종양 전체 기준)와 영역별 평균, 일치도 분포, 먼저 볼 검사 10개(일치도 낮은 순) |
| **환자 목록** | 기본 필터가 **학습에 안 쓴 검사**(검증 136개). 정렬(번호순·일치도 낮은 순·종양 큰 순), 필터(예측 완료만·절제강 있음), 환자·검사마다 일치도 점과 숫자, 종양 부피. 검사 카드의 이전/다음 버튼과 `←`/`→` 키로 목록 순서대로 이동 |
| **검사 카드** | 환자·검사, `학습에 쓰지 않은 검사`/`학습에 쓴 검사 · 참고용` 배지, **내 모델로 예측하기** 버튼과 진행률(GPU 약 10초), 연결된 모델의 검증 평균 일치도 |
| **결과 카드** | 전체 일치도(Dice)와 한 줄 판정(매우 일치·대체로 일치·부분 일치·많이 다름), 놓친/더 그린 부피, 검증 평균과 분포 속 위치(상위/하위 %), 영역별 판독/예측 부피와 일치도, 한쪽에만 있는 영역은 말로 표시, 영역 표시 토글, 마스크 `.nii.gz` 저장 |
| **뷰어** | `판독 마스크 / 내 모델 예측 / 차이 보기 / 나란히 비교` 전환. **차이 보기**는 판독∩예측 회색, 모델이 놓친 부위 파랑, 더 그린 부위 빨강을 단면과 3D에 함께 그림. 단면을 클릭하면 나머지 두 단면이 그 지점으로 이동하는 십자선. 황금비 2열: 왼쪽 결과 카드와 3D 모형(`뇌 + 종양`/`종양만 분리`, 영역 펼쳐 보기), 오른쪽 시퀀스 조절과 축상·관상·시상 단면. 나란히 비교는 3D 두 개(판독 | 예측)와 같은 단면 두 개를 함께 보여 주며, 3D 시점·자동 회전은 탭을 바꿔도 유지되고 두 3D는 함께 회전함. 영역에 마우스를 올리면 이름·부피가 뜨고, `영역 펼쳐 보기`는 조각을 벌려 라벨을 붙이며, 나침반이 R/L·A/P·S/I 방향을 표시함. 단면은 볼륨을 한 번 받아 브라우저에서 그려 휠·슬라이더가 즉시 반영됨 |
| **시점별 부피** | 검사가 여러 번인 환자는 판독 마스크 기준 시점별 부피 막대 |
| **발표 모드** | 헤더의 확대 버튼(Esc로 해제): 목록·용어·시점 막대를 숨기고 3D와 단면만 크게 |
| **용어 설명** | 검증 검사, 판독 마스크, 모델 예측, 일치도, 부피, 시퀀스, 단면, 영역 4종, 모델, 차이 보기, 놓친/더 그린 부피, 성능 개요, 상위 %, 연결된 단면 |
| **가져오기** | manifest에 없는 `.nii`/`.nii.gz` 검사를 직접 추가 |

처음에는 다음 순서로 조작해 보세요.

1. 첫 화면 **모델 성능**에서 **남은 n개 검사 전체 예측**을 누릅니다(검사당 약 5초, 136개 12분). 진행 중에도 다른 검사를 볼 수 있습니다.
2. 평균 일치도·분포·영역별 평균을 읽고, **먼저 볼 검사**에서 일치도가 가장 낮은 검사를 엽니다. 모르는 용어는 옆의 `?`를 누릅니다.
3. **차이 보기**로 모델이 놓친 부위(파랑)와 더 그린 부위(빨강)를 단면과 3D에서 확인합니다. 단면을 클릭하면 세 단면이 그 지점으로 맞춰집니다.
4. 시퀀스 버튼(T1·T1 조영·T2·FLAIR)을 바꿔 그 부위가 어떤 영상에서 보이는지 확인하고, 필요하면 **나란히 비교**로 판독과 예측을 통째로 대조합니다.
5. 검사 카드의 이전/다음(또는 `←`/`→`)으로 목록 순서대로 넘어갑니다. 왼쪽 목록을 "일치도 낮은 순"으로 정렬하면 실패 사례만 차례로 볼 수 있습니다. 다른 사람에게 보여 줄 때는 헤더의 발표 모드를 켭니다.
